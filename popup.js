...
function setDropletFill(totalMl) {
  ...
  // Calculate percentage fill for current liter (0 - 1000 mL cycle)
  const mlInCurrentLiter = totalMl % 1000;
  const progress = totalMl <= 0 ? 0 : Math.max(0, Math.min(1, mlInCurrentLiter / 1000));

  // Map progress (0 -> 1) to SVG Y coordinate (150 down to 6)
